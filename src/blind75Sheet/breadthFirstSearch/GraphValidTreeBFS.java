...
public class GraphValidTreeBFS {
    public static void main(String[] args) {
        GraphValidTreeBFS obj = new GraphValidTreeBFS();
    ...
    public boolean isTree(int n, int m, ArrayList<ArrayList<Integer>> edges) {
        // A graph with 'n' nodes and 'm' edges is a tree only if m == n - 1
        if (m != n - 1) {
            return false;
        }
        ...
        // 'nodeCount' tracks the number of nodes reached in the BFS
        int nodeCount = 1;

        // 'startNode' is the starting node for the BFS (assuming node 0)
        int startNode = 0;
        ...
        // We check if all 'n' nodes were reached (i.e., the graph is connected).
        return nodeCount == n;
    }
}
